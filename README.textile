...
* Triggers for watching files etc.
* Helpers for compiling templates etc.
...
I recommend "reading this template":http://github.com/jbe/sidekick/blob/master/lib/template. Some helpers, like @auto_compile@ set up both triggers and actions for you.
...
|@load_gem?(name)@|tries to @name@, true if success. otherwise false. informs user that more functions are available if they install it|
...
The last example is the internal implementation of the @every@ trigger seen earlier, which calls a block periodically. The @callback@ is the block given to the trigger in the @.sidekick@ file. The @timeshare@ method is a special method provided by Sidekick to all extensions, to register the supplied block so that it is called every @duration@ seconds. This is useful for polling something without blocking or forking.

Next, here is an example of the last example above written as a class:
...
* "Guard":http://github.com/guard/guard (too enterprisey)
